...
# def uploadBiData(_,info,product_id,category,industry,business_scale,user_type,no_of_users,deployment,os,mobile_apps,pricing,rating):
#     try:
#         data=BiModel.query.filter_by(product_id=product_id).first()
...
#                 pricing=pricing,
#                 rating=rating,
#                 id=encrypt_string(product_id)
#             )
#             postgredb.session.add(new_set)
#             postgredb.session.commit()
#             return new_set
#         else:
#             return GraphQLError("ProductID already present!")
#     except Exception as e:
#         return GraphQLError("error in uploading ", e)

# def getbyID(obj,info,product_id):
#     try:
#         data = BiModel.query.filter_by(product_id=product_id).first()
#         if not data:
#             raise GraphQLError("No data found for the provided ID")
...
# def getbyCategory(obj,info,category):
#     try:
#         data = BiModel.query.filter_by(category=category).all()
#         if not data:
#             return [GraphQLError("No data found for the provided category")]
